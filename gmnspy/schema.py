...
def read_config(config_file: str, data_dir: str = "", schema_dir: str = "") -> pd.DataFrame:
    ...
    resource_df["fullpath"] = resource_df["path"].apply(lambda x: os.path.join(data_dir, x))

    # Add full paths to data files
    if not schema_dir:
        schema_dir = os.path.dirname(config_file)
    resource_df["fullpath_schema"] = resource_df["schema"].apply(lambda x: os.path.join(schema_dir, x))
    print(resource_df)
