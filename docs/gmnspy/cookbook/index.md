...
For generic Frictionless data-package recipes (S3 reads, format conversion, generic spatial scope), see the [datagrove cookbook](../../datagrove/cookbook/index.md).
...
For bbox / polygon (non-GMNS-specific) spatial scope, see [datagrove cookbook: spatial scope](../../datagrove/cookbook/scope-bbox.md).
...
* [Drive both packages from a tool-call loop](../../shared/ai/json-cli.md) — every CLI command emits a parseable document with `--json`.
* [AI surface overview](../../shared/ai/index.md) — llms.txt + api-index.json + Skills + MCP.
