...
**ApprovalRequired** — exception raised when a gated operation's cost estimate exceeds the approval threshold (default 180 seconds) and the caller has not set `approve=True`. See the cost-model section of [Architecture](https://e-lo.github.io/GMNSpy/datagrove/architecture/#65-pooled-operations-cost-model).
...
* [Architecture](https://e-lo.github.io/GMNSpy/datagrove/architecture/) — design rationale for the project-convention terms.
